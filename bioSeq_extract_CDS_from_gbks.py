...
argparser.add_argument(
    ...
    help=(
        ...
        "Defaults to the first input file stem + '_etc.cds.fna'"
    ),
    default=None,
)
argparser.add_argument(
...
if args.join:
    ...
    join_path = Path(
        args.join_name if args.join_name is not None else f"{first_stem}_etc.cds.fna"
    )
    if output_dir is not None:
...
for file in args.files:
    ...
    if output_dir is not None:
        target_path = output_dir / fnaPath.name
        if fnaPath.resolve() != target_path.resolve():
            shutil.move(str(fnaPath), str(target_path))
        fnaPath = target_path

    print(fnaPath)
    if args.join:
        with open(join_path, "a") as jf, open(fnaPath) as ff:
            jf.write(ff.read())
