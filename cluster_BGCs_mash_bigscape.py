...
from pyBioinfo_modules.mash_cluster_bgc_bigscape.process_mash_result import \
    calculate_medoid
...
from pyBioinfo_modules.wrappers.mash import mashDistance, mashSketchFiles
...
def main():
    ...
    parser.add_argument(
        "--tmp", type=Path, help="temporary files folder", default=None
    )
    parser.add_argument(
        "--mashClusterRatio",
        type=float,
    ...
    parser.add_argument(
        "--verboseBigscape",
        type=bool,
        help="add --verbose to bigscape run",
        default=False,
    )
    parser.add_argument(
        "--outputPath", type=Path, help="Bigscape results", required=True
    )
    ...
    try:
        ...
        if familyMedoidFinishedFlagFile.exists():
            ...
            dict_medoids, family_distance_matrice = calculate_medoid(
                distanceTableFile, args.mashClusterRatio
            )
            if args.tmp is not None:
