...
def readPeak(file, thresh=0):
    ...
    if file.endswith(".xls"):
        ...
        data = pd.read_csv(file, delimiter="\t", usecols=cols, index_col="name")
        # TODO Thresh
    else:
        raise NameError
...
def getSeq(sourceSeq, peakDict):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for peak in peakDict:
            futures.append(
                executor.submit(slice, sourceSeq, peakDict[peak], peak)
            )
...
def pullPeakSequences(
    ...
    if multiFilter:
        ...
        filterFunction, method = singleFilter(
            filter=filter, method=method, filename=file
        )
    ...
    if os.path.isfile(output):
        ...
        SeqIO.write(peakSeqs, output, "fasta")
...
fileFilterMethod = {
    ".xls": {"fold": [0], "length": [0, 1, 2], "summit": [0], "none": [0]},
    "common_peaks.tsv": {
        "fold": [1, 2],
        "length": [0],
        "summit": [0],
...
if __name__ == "__main__":
    ...
    for file in files:
        output = os.path.join(
            outputPath, f"{os.path.splitext(os.path.split(file)[1])[0]}.fa"
        )
